...
def batched_predict(model, inp, coord, cell, bsize, config=None):
    with torch.no_grad():
        model.gen_feat(inp)
        n = coord.shape[1]
        ql = 0
...
def batched_predict_with_sf(model, inp, coord, cell, bsize, scale_factor=4):
    with torch.no_grad():
        model.gen_feat(inp, scale_factor=scale_factor)
        n = coord.shape[1]
        ql = 0
        preds = []
        while ql < n:
